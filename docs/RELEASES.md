...
2. Commit the release version and changes. Merge the intended release into the release branch first.
3. Create and push a matching version tag, for example `v0.1.1`. This is the publishing action: the added Windows release workflow runs checks and uploads the installer and metadata to GitHub Releases.
4. Confirm the release is public and contains the `.exe`, `.exe.blockmap` and `latest.yml`. Preserve all three files from the same build. A private repository needs a separate public release repository or a different distribution design; do not embed a GitHub access token in the desktop app.
...
Installed builds check shortly after startup and daily. Settings has Check for updates, Download update, automatic download preference, and Restart and install. Development builds do not download updates. Automatic downloads are optional; installation is explicit and blocked during active transfers, naming, scanning, authentication, discovery or a watcher check.
