...
function library() {
  ...
  <div class="episode-table" id="episode-list" role="tabpanel" aria-labelledby="season-${state.season}" tabindex="0"><table><thead><tr><th class="checkbox-cell"><input type="checkbox" id="select-all" ${items.length && items.every((item) => state.selected.has(item.id)) ? "checked" : ""}></th><th>EPISODE</th><th>QUALITY</th><th class="right">SIZE</th></tr></thead><tbody>${items.map((item) => `<tr data-episode-row="${escape(item.id)}" class="${state.selected.has(item.id) ? "row-selected" : ""}"><td class="checkbox-cell"><input type="checkbox" data-episode="${escape(item.id)}" aria-label="Select episode ${item.episode}" ${state.selected.has(item.id) ? "checked" : ""}></td><td><div class="episode-title" title="${escape(item.filename)}"><span class="episode-number">${pad(item.episode)}</span><div class="episode-copy"><strong>${escape(item.title)}</strong><span class="source-filename">${escape(item.filename)}</span><small>S${pad(item.season)}E${pad(item.episode)}${item.episodeEnd > item.episode ? `-E${pad(item.episodeEnd)} (combined file)` : ""} <span>·</span> ${/webrip/i.test(item.filename) ? "WEBRip" : "Video file"}${item.bitDepth ? ` <span>·</span> ${item.bitDepth}-bit` : ""}</small></div></div></td><td><span class="quality-tag">${item.resolution}p</span><span class="codec">${item.codec}</span></td><td class="file-size right" title="${item.alternatives ? `Smallest of ${item.alternatives + 1} matching files` : "Only matching file"}">${bytes(item.size)}</td></tr>`).join("")}</tbody></table>${!items.length ? '<div class="empty-inline">No matching episodes here. Try another season or quality mode.</div>' : ""}</div>
...
function settingsPage() {
  return `<section class="simple-heading"><div class="eyebrow">MAKE YOURSELF AT HOME</div><h1>A workspace that fits.</h1><p>Manage your downloads, appearance and Telegram account.</p></section><section class="settings-panel workspace-scroll" tabindex="0" role="region" aria-label="Settings">
  ...
  <div class="setting-row"><div><h3>Telegram account</h3><p>${state.connected ? `${escape(state.profile?.name || "Telegram connected")}${state.profile?.username ? ` &middot; @${escape(state.profile.username)}` : ""}. Session encrypted on this computer.` : "Connect using your own API ID and account."}</p></div><button class="button secondary" data-action="${state.connected ? "disconnect" : "connect"}">${state.connected ? "Disconnect & forget session" : "Connect Telegram"}</button></div>
