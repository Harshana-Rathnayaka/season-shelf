...
let discoveryVersion = 0, discoveryBusy = false;
...
function selectedItems() {
  return chosen().filter((item) => state.selected.has(item.id));
}
function startDemo() {
  if (!state.demo)
    liveSnapshot = {
...
function render() {
  ...
  $("#app").innerHTML = `<aside class="sidebar" id="workspace-sidebar"><button class="icon-button sidebar-toggle" data-action="toggle-sidebar" aria-controls="workspace-sidebar" aria-expanded="${!state.settings.sidebarCollapsed}" aria-label="${state.settings.sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}" title="${state.settings.sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}">${icon("sidebar")}</button>
    ...
    <div class="sidebar-bottom"><button class="nav-item ${state.page === "settings" ? "active" : ""}" data-action="nav" data-page="settings" aria-label="Settings" title="Settings">${icon("settings")}<span>Settings</span></button><button class="profile" data-action="nav" data-page="settings" aria-label="Telegram account" title="Telegram account"><span class="avatar">${icon("bolt")}</span><span><strong>${state.connected ? escape(state.profile?.name || "Telegram connected") : "Your workspace"}</strong><small>${state.demo ? "Exploring sample data" : state.connected ? escape(state.profile?.username ? "@" + state.profile.username : "Local account session") : "Connect to get started"}</small></span>${icon("chevron")}</button></div>
  </aside>
...
function qualityPicker() {
  ...
  if (!options.length) return '<div class="quality-empty">No selectable video qualities found in this scan. Check attachments needing review.</div>';
...
function library() {
  ...
  ${needsReview ? `<button class="review-note" data-action="review">${icon("info")} ${needsReview} ${needsReview === 1 ? "attachment needs review and was" : "attachments need review and were"} not automatically selected.</button>` : ""}
  ${
    state.catalogue
      ? `<div class="library-controls">${qualityPicker()}<div class="catalogue-tools"><div class="season-navigation"><button class="icon-button season-prev" data-action="scroll-seasons" data-direction="-1" aria-label="Earlier seasons">${icon("chevron")}</button><div class="season-tabs" id="season-tabs" role="tablist" aria-label="Seasons">${seasons.map((season) => `<button role="tab" aria-controls="episode-list" id="season-${season}" tabindex="${state.season === season ? 0 : -1}" aria-selected="${state.season === season}" class="season-tab ${state.season === season ? "active" : ""}" data-action="season" data-season="${season}">Season ${pad(season)}<span>${all.filter((item) => item.season === season).length}</span></button>`).join("")}</div><button class="icon-button season-next" data-action="scroll-seasons" data-direction="1" aria-label="Later seasons">${icon("chevron")}</button></div><label class="table-search">${icon("search")}<input id="episode-search" placeholder="Find an episode…" aria-label="Find an episode" value="${escape(state.query)}"></label></div></div>
  <div class="episode-table" id="episode-list" role="tabpanel" aria-labelledby="season-${state.season}" tabindex="0"><table><thead><tr><th class="checkbox-cell"><input type="checkbox" id="select-all" ${items.length && items.every((item) => state.selected.has(item.id)) ? "checked" : ""}></th><th>EPISODE</th><th>QUALITY</th><th class="right">SIZE</th></tr></thead><tbody>${items.map((item) => `<tr data-episode-row="${escape(item.id)}" class="${state.selected.has(item.id) ? "row-selected" : ""}"><td class="checkbox-cell"><input type="checkbox" data-episode="${escape(item.id)}" aria-label="Select episode ${item.episode}" ${state.selected.has(item.id) ? "checked" : ""}></td><td><div class="episode-title" title="${escape(item.filename)}"><span class="episode-number">${pad(item.episode)}</span><div class="episode-copy"><strong>${escape(item.title)}</strong><span class="source-filename">${escape(item.filename)}</span><small>S${pad(item.season)}E${pad(item.episode)} <span>·</span> ${/webrip/i.test(item.filename) ? "WEBRip" : "Video file"}${item.bitDepth ? ` <span>·</span> ${item.bitDepth}-bit` : ""}</small></div></div></td><td><span class="quality-tag">${item.resolution}p</span><span class="codec">${item.codec}</span></td><td class="file-size right" title="${item.alternatives ? `Smallest of ${item.alternatives + 1} matching files` : "Only matching file"}">${bytes(item.size)}</td></tr>`).join("")}</tbody></table>${!items.length ? '<div class="empty-inline">No matching episodes here. Try another season or quality mode.</div>' : ""}</div>
  <div class="selection-bar"><div><strong>${selected.length} ${selected.length === 1 ? "episode" : "episodes"} selected</strong><span>${bytes(selected.reduce((n, item) => n + item.size, 0))} total ${new Set(selected.map((item) => item.season)).size > 1 ? "across seasons" : ""}</span></div><div class="selection-actions"><button class="text-button" data-action="select-season">Select season</button><button class="button primary" data-action="download" ${!selected.length || state.busy ? "disabled" : ""}>${icon("download")}${state.demo ? "Preview download queue" : "Download selected"}${icon("arrow")}</button></div></div>`
  ...
  <div class="destination-strip"><span class="destination-icon">${icon(state.mode === "archive" ? "drive" : "folder")}</span><div><span class="destination-label">${state.mode === "archive" ? "ARCHIVE DESTINATION" : "TEMPORARY VIEWING FOLDER"}</span><strong>${escape(root?.path || "Choose a folder before downloading")}</strong></div><span class="destination-hint">${state.mode === "archive" ? "Sorted into season folders" : "Stays separate from your archive"}</span><button class="text-button" data-action="folder" data-mode="${state.mode}">${root ? "Change" : "Choose folder"}${icon("arrow")}</button></div>`;
}
...
function queuePage(saved) {
  const jobs = saved
    ? state.jobs.filter((job) => ["complete", "deleted"].includes(job.status))
  ...
  <div class="queue-summary"><div><small>IN QUEUE</small><strong>${jobs.filter((j) => !["complete", "deleted", "cancelled"].includes(j.status)).length}</strong></div><div><small>${saved ? "COMPLETED" : "BATCH COMPLETED"}</small><strong>${jobs.filter(j => j.status === "complete" && (saved || j.batchId === state.currentBatchId)).length}</strong></div><div>${saved ? `<small>SAVED FILE SIZE</small><strong class="size-total">${bytes(jobs.filter(j => j.status === "complete").reduce((n,j) => n+j.item.size,0))}</strong>` : `<small>CURRENT BATCH</small><strong class="size-total">${bytes(received)} / ${bytes(total)}</strong><small>${bytes(remaining)} remaining</small><progress class="batch-progress" max="${total || 1}" value="${received}" aria-label="Current batch download progress"></progress>`}</div></div>
  ${!saved && jobs.length ? `<div class="bulk-controls">${[["pause", "Pause all"], ["resume", "Resume all"], ["cancel", "Cancel all"]].map(([action, label]) => `<button class="button secondary" data-action="bulk-job" data-control="${action}">${label}</button>`).join("")}<small>Checking and transferring files finish first. Cancel keeps partial files.</small></div>` : ""}
  ${jobs.length ? `<div class="bulk-delete-row"><button class="button secondary" data-action="${saved ? "delete-all-saved" : "delete-all-queue"}">${icon("trash")}${saved ? "Delete all saved files" : "Delete all pending"}</button><small>${saved ? "Moves Archive and Watch files to the Recycle Bin after confirmation." : "Removes unfinished entries; completed media and partial files are kept."}</small></div>` : ""}
  <section class="catalogue-panel queue-panel workspace-scroll" tabindex="0" role="region" aria-label="${saved ? "Saved files" : "Downloads"}">${jobs.length ? jobs.map((job) => `<article class="queue-item"><span class="queue-glyph">${icon(job.mode === "archive" ? "drive" : "play")}</span><div class="queue-content"><div class="queue-title"><strong>${escape(job.series)} <span>S${pad(job.item.season)}E${pad(job.item.episode)}</span></strong><span class="status status-${job.status}">${escape(job.status)}</span></div><span class="source-filename queue-filename" title="${escape(job.finalPath ? job.finalPath.split(/[\\/]/).pop() : job.item.filename)}">${escape(job.finalPath ? job.finalPath.split(/[\\/]/).pop() : job.item.filename)}</span><p>${job.item.resolution}p ${job.item.codec} <span>·</span> ${job.mode === "archive" ? "Archive" : "Watch"} <span>·</span> ${bytes(job.received || 0)} / ${bytes(job.item.size)}${job.speed ? ` <span>·</span> ${bytes(job.speed)}/s` : ""}</p><progress max="${job.item.size}" value="${job.received || 0}" aria-label="Download progress"></progress>${job.error ? `<small class="job-error">${escape(job.error)}</small>` : ""}${job.namingError ? `<small class="job-error">Filename normalization: ${escape(job.namingError)}</small>` : ""}${job.finalPath ? `<button class="text-button file-details-link" data-action="file-details" data-job="${job.id}">File details ${icon("info")}</button>` : ""}</div><div class="job-controls">${!["checking", "transferring", "complete", "deleted"].includes(job.status) ? `<button class="icon-button" data-action="job" data-job="${job.id}" data-control="remove" aria-label="Remove from queue" title="Remove from queue; partial file retained">${icon("trash")}</button>` : ""}${["queued", "downloading", "retrying", "preview"].includes(job.status) ? `<button class="icon-button" data-action="job" data-job="${job.id}" data-control="pause" aria-label="Pause download">${icon("pause")}</button>` : ""}${["paused", "failed", "waiting", "cancelled"].includes(job.status) ? `<button class="icon-button" data-action="job" data-job="${job.id}" data-control="resume" aria-label="Resume download">${icon("play")}</button>` : ""}${["queued", "downloading", "paused", "retrying", "preview"].includes(job.status) ? `<button class="icon-button" data-action="job" data-job="${job.id}" data-control="cancel" aria-label="Cancel download">${icon("close")}</button>` : ""}${job.status === "complete" ? `<button class="icon-button" data-action="open-job" data-job="${job.id}" aria-label="Open video">${icon("play")}</button><button class="icon-button" data-action="delete-job" data-job="${job.id}" aria-label="Move downloaded file to Recycle Bin" title="Move to Recycle Bin">${icon("trash")}</button>` : ""}</div></article>`).join("") : `<div class="empty-state"><div class="empty-symbol">${icon(saved ? "folder" : "download")}</div><h3>${saved ? "Your collection is waiting." : "Nothing in the queue. Yet."}</h3><p>Choose episodes from your series library to get started.</p><button class="button primary" data-action="nav" data-page="library">Browse your series ${icon("arrow")}</button></div>`}</section>`;
}
function helpPage() {
  return `<section class="simple-heading"><div class="eyebrow">YOUR QUICK GUIDE</div><h1>From channel to season folder.</h1><p>A few choices now, less clicking later.</p></section><section class="settings-panel help-panel workspace-scroll" tabindex="0" role="region" aria-label="How it works"><h2>How to use Season Shelf</h2><ol><li>Connect Telegram with your API ID, hash and phone number. Complete Telegram's sign-in prompts.</li><li>Choose a series channel you already belong to, then scan its episode files. Suggested channels hides likely finance/crypto names and puts media names first. Unknown show names remain visible; All channels restores every result. This is a name-based filter, not content verification.</li><li>Choose Archive or Watch, then a quality and codec from the compact buttons. Only valid qualities found in the scanned channel appear; availability can differ by season. Auto codec prefers HEVC. Archive defaults to 720p HEVC. The smallest matching file per episode is selected.</li><li>Choose the show's folder, for example <code>D:/Shows/12 Monkeys</code>. New downloads go straight into <code>Season 02</code> inside it. Choose separate Archive and Watch folders.</li><li>Select episodes or a whole season and click Download selected.</li><li>Use Downloads to track downloaded/total size, pause, resume, cancel or remove entries. Bulk actions affect eligible unfinished jobs. Verification and transfer finish first.</li></ol><h2>What happens to your files?</h2><p>New files keep their original names while downloading. Once all queued files for a season finish, the majority dot, underscore or dash style is applied to the other completed files from that season. Ties keep original names; extensions, decimals and recognised compound tags such as WEB-DL are preserved. This uses completed app-managed files, not unverified broadcast-season completeness. Characters Windows cannot store are sanitized. Existing queued jobs keep their saved paths.</p><p>Files download to local staging, then are copied and checked before the staged copy is removed. Keep enough free space on your PC and destination. Different existing files are never overwritten.</p><p>Cancel retains progress for resume. Remove hides a queue entry and retains its partial file on disk; adding it again starts a new job. Completed media remains in Saved files. Delete all pending clears unfinished queue entries but retains partial files. Settings can find orphaned partials and move them to the Recycle Bin. Delete all saved files moves completed Archive and Watch files to the Recycle Bin after confirmation. Completed Archive and Watch files can be moved to the Recycle Bin after confirmation.</p><h2>Restart and recovery</h2><p>Keep the app open while downloading. After restarting, your encrypted Telegram session reconnects automatically. Resume paused jobs when ready. Sign in again if the session has expired or been revoked. Reconnect a missing HDD using the same drive letter, then resume. Downloads cannot continue while the PC is shut down.</p><h2>Audio, subtitles and discovery</h2><p>Check the full source filename and play a sample for language/subtitles. These details and bitrate are not reliably available from channel metadata. Use Change series to find the MovieClubFamily search group. Post a series name there and select the bot's reply, or read a pasted private message link. Select a result, start the bot if needed, then preview and join the series channel. Callback pagination and required subscription tasks still need Telegram. Automatic new-episode monitoring is planned.</p></section>`;
}
function usageTiles() {
...
function settingsPage() {
  return `<section class="simple-heading"><div class="eyebrow">MAKE YOURSELF AT HOME</div><h1>A workspace that fits.</h1><p>Your folders, your pace, your preferred shade of dark.</p></section><section class="settings-panel workspace-scroll" tabindex="0" role="region" aria-label="Settings">
  <div class="setting-row"><div><h3>Appearance</h3><p>A little easier on the eyes.</p></div><div class="segmented">${["dark", "light", "system"].map((theme) => `<button data-action="set-theme" data-theme="${theme}" class="${state.settings.theme === theme ? "active" : ""}">${theme[0].toUpperCase() + theme.slice(1)}</button>`).join("")}</div></div>
  ${appearanceForm(state.settings)}
  ${["archive", "watch"].map((mode) => `<div class="setting-row"><div><h3>${mode === "archive" ? "Archive destination" : "Viewing folder"}</h3><p>${escape(state.settings[`${mode}Root`]?.path || "No folder selected")}</p></div><button class="button secondary" data-action="folder" data-mode="${mode}">${icon("folder")}Choose folder</button></div>`).join("")}
  <div class="setting-row"><div><h3>Simultaneous downloads</h3><p>Choose how many files can download at once.</p></div><select id="concurrency" aria-label="Simultaneous downloads">${[1, 2, 3, 4].map((n) => `<option ${n === state.settings.concurrency ? "selected" : ""}>${n}</option>`).join("")}</select></div>
  <div class="setting-row"><div><h3>Telegram account</h3><p>${state.connected ? `${escape(state.profile?.name || "Telegram connected")}${state.profile?.username ? ` ? @${escape(state.profile.username)}` : ""}. Session encrypted on this computer.` : "Connect using your own API ID and account."}</p></div><button class="button secondary" data-action="${state.connected ? "disconnect" : "connect"}">${state.connected ? "Disconnect & forget session" : "Connect Telegram"}</button></div>
  <div class="keyword-settings"><h3>Channel keyword filter</h3><p>Hide matching words or phrases in Suggested channels. One per line or separate with commas. All channels always shows everything.</p><form id="keyword-form"><label for="hidden-keywords">Hidden keywords</label><textarea id="hidden-keywords" name="keywords" rows="5" spellcheck="false">${escape((state.settings.hiddenKeywords ?? defaultHiddenKeywords).join("\n"))}</textarea><div class="row-actions"><button class="button primary" type="submit">Save keywords</button><button class="button secondary" data-action="reset-keywords">Restore defaults</button></div></form><small>Media-labelled channels stay visible. Leave the list empty to disable keyword hiding.</small></div>
  <div class="keyword-settings"><h3>Unused partial files</h3><p>${state.stagingInfo ? `${state.stagingInfo.count} unused ${state.stagingInfo.count === 1 ? "file" : "files"} ? ${bytes(state.stagingInfo.bytes)}` : "Check for partial files left behind by removed queue entries."}</p><div class="row-actions"><button class="button secondary" data-action="scan-staging">Check storage</button><button class="button secondary" data-action="cleanup-staging" ${!state.stagingInfo?.count ? "disabled" : ""}>${icon("trash")}Clean up unused partials</button></div><p>Current jobs, including paused and cancelled entries, are preserved. Cleanup uses the Recycle Bin.</p></div>
  <div class="usage-settings"><h3>Lifetime activity</h3><div class="usage-summary" id="usage-summary">${usageTiles()}</div><p>Measured since ${state.usage.since ? escape(new Date(state.usage.since).toLocaleString()) : "this update"}. Received data includes retries. Saved data counts successfully completed downloads. These totals remain after deleting files and exclude activity before tracking began.</p></div>
  <div class="settings-footnote">${icon("shield")} Files download directly from Telegram. Settings and download history stay on your computer. This app uses the Telegram API and is an independent, unofficial client.</div></section>`;
}
...
async function discoveryRequest(method,payload) {
  ...
  try {
    ...
    modal(`<h2>Discovery results</h2><div class="discovery-results">${result.messages.map(message=>`<section class="discovery-message"><p>${escape(message.text)}</p>${message.links.map(link=>`<button class="channel-option" data-action="discovery-follow" data-choice="${escape(link.id)}"><span>${escape(link.label)}<small>${link.kind === "bot-start" ? "Start bot with this result" : link.kind === "private-message" ? "Read linked message" : "Preview Telegram destination"}</small></span>${icon("arrow")}</button>`).join("")}</section>`).join("") || '<p>No reply arrived. Try again later or open the bot in Telegram.</p>'}</div><p>Only supported Telegram links appear. Callback pages, files sent directly by bots, and subscription tasks currently need Telegram.</p><button class="button secondary" data-action="discover">New search</button>`);
  } catch (error) {
    if (version === discoveryVersion) throw error;
...
document.addEventListener("click", async (event) => {
  ...
  try {
    if (action === "toggle-sidebar") {
      ...
      const items = chosen().filter((i) => i.season === state.season);
      ...
      const result = await call("discovery-join",{id:el.dataset.choice});
      state.channels = result.channels;
      await scan(result.channel.id);
      ...
      modal(
        `<h2>Attachments to review.</h2><p>These files were excluded because their episode, resolution or codec could not be identified reliably. WEB.x264 identifies a codec, not a resolution.</p><p class="review-explanation">This list is informational. Check the source in Telegram for a clearly labelled release. Manual corrections are planned; files are never guessed into your download selection.</p><div class="review-list">${state.catalogue.items
          .filter((i) => i.reason)
          .map(
            (i) =>
              `<article><strong>${escape(i.filename)}</strong><p>${escape(i.reason)}</p></article>`,
          )
          .join("")}</div>`,
      );
  } catch (error) {
  ...
  window.shelf.on(({ type, data }) => {
    if (type === "connection") {
      ...
      if (state.demo) {
        ...
        state.jobs = data;
        if (state.page === "queue" || state.page === "saved") render();
      }
      ...
      modal(
        `<div class="modal-icon">${icon("shield")}</div><h2>${escape(data.label)}</h2><p>Enter it here on your computer. It is never saved in your download history.</p><form id="auth-form"><label>${escape(data.kind)}<input name="value" required autocomplete="off" type="${data.kind === "password" ? "password" : "text"}" autofocus></label><button class="button primary full" type="submit">Continue ${icon("arrow")}</button></form>`,
      );
