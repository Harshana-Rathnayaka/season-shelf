...
export function DiscoveryDialog({ view, onAction, onSubmit }: DialogCallbacks & { view: View }) {
  ...
  switch (view.kind) {
    ...
    case "discovery-error": return <><h2>Could not finish opening the series.</h2><p role="alert">{view.error}</p><p>If Telegram already joined the channel, select it from your channels to load its episodes.</p><div className="dialog-actions"><DialogButton label="Choose series channel" className="button primary" action="choose-series" onAction={onAction} /><DialogButton label="Try search again" action="discover" onAction={onAction} /></div></>;
    case "discovery-results": return <><h2>Discovery results</h2>{view.notice && <p role="status">{view.notice}</p>}<div className="discovery-results">
      ...
      </div><p>A single series result continues automatically. Choose a result when there are several. Subscription tasks require your selection.</p><div className="dialog-actions"><DialogButton label="Bot requires channel subscriptions?" action="subscription-help" onAction={onAction} /><DialogButton label="New search" action="discover" onAction={onAction} /></div>
    </>;
